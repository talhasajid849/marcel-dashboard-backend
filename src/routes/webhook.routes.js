...
router.post('/stripe', async (req, res) => {
  ...
  try {
    ...
    if (!eventType && req.body.session_id) {
      await handleCheckoutCompleted({ id: req.body.session_id, metadata: {} });
    }
...
async function handleCheckoutCompleted(session) {
 ...
 // Update customer stats + tier
  ...
  if (customer) {
    ...
    customer.total_spent         = (customer.total_spent || 0) + (booking.amount_upfront || 0);
    ...
    if (booking.licence_photo_front_url) customer.licence_photo_front_url = booking.licence_photo_front_url;
    if (booking.licence_photo_back_url)  customer.licence_photo_back_url  = booking.licence_photo_back_url;
    ...
    if (booking.country_of_origin)       customer.country_of_origin = booking.country_of_origin;
    if (booking.next_of_kin)             customer.next_of_kin       = booking.next_of_kin;
    if (booking.next_of_kin_phone)       customer.next_of_kin_phone = booking.next_of_kin_phone;
    if (booking.licence_type)            customer.licence_type      = booking.licence_type;

    await customer.save();

    console.log(`✅ Customer updated: ${customer.name} | Tier: ${customer.customer_tier} | Hires: ${hires}`);
  }
  ...
  if (stripeCustomerId && paymentIntentId) {
    try {
      ...
      if (paymentMethodId) {
        ...
        console.warn('⚠️  No payment method on PaymentIntent — subscription not created');
      }
    } catch (err) {
      console.error('⚠️  Subscription creation error (booking still confirmed):', err.message);
    }
  }
...
async function handleInvoicePaymentSucceeded(invoice) {
  ...
  console.log(`✅ Weekly payment received: $${amountPaid} for subscription ${stripeSubId}`);

  // Find our subscription
  const subscription = await Subscription.findOne({ stripe_subscription_id: stripeSubId });
  if (!subscription) {
    console.warn('⚠️  No local subscription for Stripe subscription:', stripeSubId);
    return;
  }

  // Find next unpaid week and mark it paid
  const unpaidWeek = subscription.weekly_payments.find(p => p.status === 'PENDING');
  if (unpaidWeek) {
    await subscription.markWeekPaid(unpaidWeek.week_number, invoice.id, 'AUTO_CHARGE');
  ...
  if (subscription.customer_whatsapp_id) {
    try {
      ...
      await platformMessenger.sendMessage('whatsapp', subscription.customer_whatsapp_id, msg);
    } catch (e) {
...
async function handleInvoicePaymentFailed(invoice) {
  ...
  try {
    ...
    await platformMessenger.sendMessage('whatsapp', colePhone,
    ...
    if (subscription.customer_whatsapp_id) {
      await platformMessenger.sendMessage('whatsapp', subscription.customer_whatsapp_id,
...
async function sendConfirmationMessage(booking) {
  ...
  try {
    ...
    const upfront     = booking.amount_upfront || (weeklyRate + deposit + deliveryFee);
